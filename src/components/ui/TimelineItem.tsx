    ...
    <div className="relative pl-8 pb-12">
      ...
      {!isLast && (
        <div 
          ...
          style={{
            background: "linear-gradient(to bottom, #DC2626, rgba(220, 38, 38, 0.5), transparent)"
          }}
        />
      )}
      ...
      >
        <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4 gap-2">
          <div>
            <h3 className="text-xl font-display font-bold mb-1" style={{ color: "#FFFFFF" }}>
              {title}
            </h3>
            {subtitle && (
              <p className="font-body font-semibold text-lg" style={{ color: "#DC2626" }}>
                {subtitle}
              </p>
            )}
          </div>
        ...
        <ul className="space-y-2">
          {points.map((point, i) => (
            <li
              ...
              style={{ color: "#9CA3AF" }}
            >
              <span className="mr-3 mt-1.5 font-bold" style={{ color: "#DC2626" }}>▸</span>
              <span>{point}</span>
            </li>
          ))}
        </ul>
